...
//                     cout << curTick <<": allocating thread\n";
    ...
    static void
    printComBugTrace(std::string message){
        ofstream bugtrace("communicationBugTrace.txt", ofstream::app);
        bugtrace << curTick << ": " << message << "\n";
        bugtrace.flush();
        bugtrace.close();
    }
	...
	uint64_t usecs = xc->getSyscallArg(4);
        ...
        //HACK: must be here for it to compile (by Magnus)
        if(false){
...
//         if(curTick >= 6000000){
//             cout << curTick << ": Waitlist is now (1):";
//             for(list<Process::WaitRec>::iterator a = process->waitList.begin();a != process->waitList.end(); a++){
//                 ExecContext* tmp = a->waitingContext;
//                 int tmpId = tmp->cpu->params->cpu_id;
//                 cout << "(" << tmpId << ", " << a->waitChan << ")";
//             }
//             cout << "\n";
//         }
...
//         if(curTick > 6000000) cout << "calling from m5_unlock_mutex\n";
...
//         if(curTick > 6000000) cout << "calling from broadcast, addr is " << cond_addr << "\n";
...
//             warn("The thread sheduler has deadlocked, releasing one processor");
            printComBugTrace("The thread sheduler has deadlocked, releasing one processor");
