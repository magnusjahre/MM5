...
protected:
	...
	struct cacheOccupancy {
		...
		cacheOccupancy(Tick _start, Tick _end, int _id, Tick _origReq) :
			startTick(_start), endTick(_end), occCPUID(_id),
					originalRequestTick(_origReq) {
		}

		std::string toString() {
			std::stringstream retstr;
			retstr << "Occupied by CPU" << occCPUID << " from " << startTick
					<< " to " << endTick << ", first req at "
					<< originalRequestTick;
			return retstr.str();
		}
	};
...
public:
	...
	public:
		 ...
		 */
		Params(std::vector<Range<Addr> > addr_range, int hit_latency,
				int _blkSize, Counter max_misses, int _baseCacheCPUCount) :
			addrRange(addr_range), hitLatency(hit_latency), blkSize(_blkSize),
					maxMisses(max_misses),
					baseCacheCPUCount(_baseCacheCPUCount) {
		}
	};
...
//	virtual void handleRepartitioningEvent() = 0;
	...
	virtual CacheBlk::State getNewCoherenceState(MemReqPtr &req,
			CacheBlk::State old_state) = 0;
